...
/**
 * NemotronApiClient handles cloud AI inference using NVIDIA Nemotron models.
...
object NemotronApiClient {
    ...
    /**
     * Sends the text and specific PolishMode to Nemotron API.
     */
            ...
            try {
                ...
                if (response.isSuccessful && !responseString.isNullOrEmpty()) {
                    ...
                    if (choices != null && choices.length() > 0) {
                        ...
                        val rawTextResult = message?.optString("content", "")?.trim() ?: ""
                    ...
                    Log.d(TAG, "Nemotron API ($model) response code: ${response.code}, body: $responseString")
                }
            } catch (e: Exception) {
                Log.d(TAG, "Nemotron API ($model) request failed: ${e.message}")
            }
        }
